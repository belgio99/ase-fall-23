...
def ping(URL):
    try:
        r = subprocess.run(f'ping -w 2 -c 3 {URL}', capture_output=True, shell=False)
...
def crashLog():
    try:
        x = requests.get(LOG_URL + f'/crash', timeout=60)
        x.raise_for_status()
        return x.json()
    except ConnectionError:
        return make_response('Log service is down\n', 404)
...
def getLogs():
    try:
        x = requests.get(LOG_URL + f'/getLogs', timeout=60)
        x.raise_for_status()
        return x.json()
    except ConnectionError:
        return make_response('Log service is down\n', 404)
